...
with open(endereco,'w') as arquivo:
    ...
    for geracao in range(0, num_geracoes):
        ...
        for b in P[geracao]:
            arquivo.write(f'Rank ( {b.rank} ) | ISP = {round(b.isp, 2)} s | Cf = {round(b.Cf, 2)} | cstar = {round(b.cstar, 2)} m/s | F = {round(b.empuxo)} N | O/F = {round(b.genes[0], 2)} | Pc = {round(b.genes[1], 1)} bar | Pressão na saída do bocal = {round(b.genes[3], 3)} bar| Temp = {round(b.temperaturaCC)} K | dt = {round(b.genes[2], 1)} mm | R_exp = {round(b.Razao_Expansao, 1)} | tb = {round(b.t_burn, 1)} s ' + '\n')
